...
function openCommunityMineCreationDetail(){
  goTo('sc-community-creation-detail');
}

function closeCommunityCreationDetail(){
  goTo('sc-community');
  setCommunityTab('mine');
}

function shareCreationFromDetail(platform){
  showToast('🚀 已唤起分享至「' + platform + '」');
}
...
function renderCommunityMineOutputs(){
  ...
  list.innerHTML = studioState.outputs.slice(0, 6).map(function(item){
    ...
    var mediaHtml = item.type === 'post'
      ? '<img src="' + preview + '" alt="我的创作缩略图" loading="lazy" decoding="async" style="width:100%;height:168px;border-radius:10px;object-fit:cover;display:block;"/>'
      : '<div style="width:100%;height:168px;border-radius:10px;overflow:hidden;background:#000;"><video src="' + preview + '" muted playsinline loop autoplay preload="metadata" style="width:100%;height:100%;object-fit:cover;display:block;pointer-events:none;" aria-label="我的创作视频"></video></div>';
    var wrapOpen = item.type === 'post'
      ? '<div style="background:#fff;border:1px solid rgba(0,0,0,0.06);border-radius:14px;padding:8px;">'
      : '<div role="button" tabindex="0" class="community-mine-card--tap" onclick="openCommunityMineCreationDetail()" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();openCommunityMineCreationDetail();}" style="background:#fff;border:1px solid rgba(0,0,0,0.06);border-radius:14px;padding:8px;cursor:pointer;">';
    return wrapOpen + mediaHtml + '<div style="padding:8px 4px 2px;"><div class="profile-menu-name">' + item.title + '</div><div class="profile-menu-sub">' + subtitle + '</div></div><div style="display:flex;justify-content:flex-end;"><span class="profile-menu-badge" style="background:' + badgeBg + ';color:' + badgeColor + ';">' + badgeText + '</span></div></div>';
  }).join('');
}
...
function renderStudioOutputs(){
  ...
  if (previewMedia) {
    if (top.type === 'post') {
      ...
      previewMedia.innerHTML = '<div style="position:relative;width:100%;height:100%;border-radius:10px;overflow:hidden;background:#000;"><video src="assets/河流狗.mp4" muted playsinline loop autoplay preload="metadata" style="width:100%;height:100%;object-fit:cover;display:block;" aria-label="视频生成结果预览"></video><span style="position:absolute;right:10px;bottom:10px;background:rgba(0,0,0,0.55);color:#fff;font-size:10px;font-weight:800;padding:3px 7px;border-radius:10px;">▶ ' + (studioState.duration || 15) + 's</span></div>';
    }
  }
