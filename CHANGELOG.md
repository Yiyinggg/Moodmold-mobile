...
- Home 使用 **`assets/home dog.png`** 作为主体小狗图（路径含空格，HTML 中使用 `home%20dog.png`）。
- 增加 **`assets/`** 资源目录；保留 **`pet-home.png`** 等历史资源文件。
- 首页主体区：浅色圆角「舞台」、双圆环、星标/爱心；小狗图放大展示，名称区大写标题样式。
- 全屏高度与滚动与底部导航的间距已做适配（`scroll-padding-bottom` 等）。

### 初始版本（`46aa079`）

- 单文件原型 **`moodmol.html`**：Home、Play/AR、Feed、Dress、Create、Generating 等界面。
